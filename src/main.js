...
async function transferPlaylist(spListId, ytListId) {
  ...
  for (let i = 0; i < tracks.length; i++) {
    ...
    const ytResult = await youtube.search(
...
async function downloadPlaylist(ytListId) {
  ...
  playlist.items.forEach(async track => {
    ...
    if (
      playlist.items.find(t => t.snippet.resourceId.videoId === id) !== track
    ) {
      // If the same track appears somewhere before this one in the playlist, remove it
    ...
    if (
      queue.some(
        t => (t.ytId === id && t.state === "down") || t.state === "extract"
      )
    ) {
      return;
    }
...
async function downloadVideo(track, outfile) {
  ...
  info.formats.forEach(fmt => {
    ...
    if (
      fmt.audioEncoding === "aac" &&
      (!format || fmt.audioBitrate > format.audioBitrate)
    ) {
      format = fmt;
    }
  });
...
function extractAudio(infile, outfile, tags = {}) {
  ...
  return new Promise((resolve, reject) => {
    shell.exec(
      `${alias} -i ${infile} -y -vn -acodec copy -metadata artist="${tags.artist ||
        ""}" -metadata title="${tags.title ||
        ""}" -metadata genre="${tags.genre || ""}" "${outfile}"`,
      { silent: true },
      (code, stdout, stderr) => {
