...
if (key.client_id && key.client_secret) {
  ...
  console.error(
    "No usable Spotify key found. Please get your API credentials from Spotify for Developers."
  );
}
...
module.exports = {
  ...
  list: (username, listId) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      spotify.getPlaylist(username, listId).then(
        data => {
        ...
        err => {
          ...
          if (err.statusCode === 401) {
            spotify.refreshAccessToken().then(data => {
              ...
              spotify.getPlaylist(username, listId).then(data => {
                resolve(data.body.tracks.items.map(track => track.track));
              }, reject);
            }, reject);
          } else {
  ...
  remove: (username, listId, tracks) => {
    return new Promise((resolve, reject) => {
      spotify
        .removeTracksFromPlaylist(username, listId, tracks, {})
        .then(resolve, err => {
          // If we are unauthorized, refresh the access token and retry
          if (err.statusCode === 401) {
            spotify.refreshAccessToken().then(data => {
